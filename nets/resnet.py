...
def Rk(x, name):
   ...
   # layer 1
   conv1 = tf.layers.conv2d(padded_x, 128, 3, strides=1, name='g-'+name+'-1', padding='VALID')
   ...
   # layer 2
   conv2 = tf.layers.conv2d(padded_conv1, 128, 3, strides=1, name='g-'+name+'-2', padding='VALID')
...
def netG(inputs, direction):
   ...
   # c7s1-32
   conv1 = tf.layers.conv2d(padded_inputs, 32, 7, strides=1, name='g-conv1-'+direction, padding='VALID')
   conv1 = instance_norm(conv1)
   conv1 = tf.nn.relu(conv1)
   print 'conv1:',conv1

   # d64
   conv2 = tf.layers.conv2d(conv1, 64, 3, strides=2, name='g-conv2-'+direction, padding='SAME')
   conv2 = instance_norm(conv2)
   conv2 = tf.nn.relu(conv2)
   print 'conv2:',conv2
   
   # d128
   conv3 = tf.layers.conv2d(conv2, 128, 3, strides=2, name='g-conv3-'+direction, padding='SAME')
   conv3 = instance_norm(conv3)
   conv3 = tf.nn.relu(conv3)
   print 'conv3:',conv3

   # R128
   r1 = Rk(conv3,'g-'+direction+'-res-0')
   print 'r1:',r1
   r2 = Rk(r1,'g-'+direction+'-res-1')
   print 'r2:',r2
   r3 = Rk(r2,'g-'+direction+'-res-2')
   print 'r3:',r3
   r4 = Rk(r3,'g-'+direction+'-res-3')
   print 'r4:',r4
   r5 = Rk(r4,'g-'+direction+'-res-4')
   print 'r5:',r5
   r6 = Rk(r5,'g-'+direction+'-res-5')
   print 'r6:',r6
   r7 = Rk(r6,'g-'+direction+'-res-6')
   print 'r7:',r7
   r8 = Rk(r7,'g-'+direction+'-res-7')
   print 'r8:',r8
   r9 = Rk(r8,'g-'+direction+'-res-8')
   print 'r9:',r9

   t_conv1 = tf.layers.conv2d_transpose(r9, 64, 3, strides=2, name='g-t_conv1-'+direction, padding='SAME')
   t_conv1 = instance_norm(t_conv1)
   t_conv1 = tf.nn.relu(t_conv1)
   print 't_conv1:',t_conv1
   
   t_conv2 = tf.layers.conv2d_transpose(t_conv1, 3, 3, strides=2, name='g-t_conv2-'+direction, padding='SAME')
   t_conv2 = instance_norm(t_conv2)
